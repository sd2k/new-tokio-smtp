...
use new_tokio_smtp::chain::{OnError, HandleErrorInChain};
...
fn runs_the_cmd_chain() {
    ...
    let chain =
        smtp_chain!(con with OnError::StopAndReset => [
...
fn stops_on_error() {
    ...
    let chain =
        smtp_chain!(con with OnError::Stop => [
            command::Verify { query: "test1".to_owned() },
            command::Verify { query: "test2".to_owned() },
            command::Verify { query: "test3".to_owned() }
        ]).and_then(|(con, res)| {
...
fn ignores_error_if_requested() {
    ...
    let chain =
        smtp_chain!(con with IgnoreAllErrors => [
            command::Verify { query: "test1".to_owned() },
            command::Verify { query: "test2".to_owned() },
            command::Verify { query: "test3".to_owned() }
        ]).and_then(|(con, res)| {
